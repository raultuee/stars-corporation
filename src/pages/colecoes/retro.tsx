        ...
        {retroItems.length > 0 && (
          <Card className="group w-[320px] h-[400px] flex flex-col items-center justify-center transition duration-300 hover:scale-105 hover:shadow-lg relative">
            <div className="relative w-[220px] h-[220px] mb-4">
              <img
                src={retroItems[current].imagem}
                alt={retroItems[current].nome}
                className={
                  retroItems[current].imagemSec
              ...
              {retroItems[current].imagemSec && (
                <img
                  src={retroItems[current].imagemSec}
                  alt={retroItems[current].nome + ' costas'}
                  className="absolute inset-0 w-full h-full object-cover transition-opacity duration-300 opacity-0 group-hover:opacity-100"
                />
              )}
            </div>
            <h2 className="text-xl font-semibold text-center mt-5 mb-5">{retroItems[current].nome}</h2>
            <div className="flex gap-2 items-center">
              <Button
                className="w-[150px]"
                onClick={() => window.location.href = `/produto/${retroItems[current].slug}`}
              >
                {retroItems[current].preco.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
              </Button>
              <Button
                onClick={() => handleLike(retroItems[current].id)}
                variant={isLiked[retroItems[current].id] ? "default" : "secondary"}
                disabled={isLiked[retroItems[current].id]}
              >
                <Heart />
              </Button>
              <Badge className="ml-2 hidden md:block lg:block xl:block" variant="secondary">
                  {likes[retroItems[current].id] || 0} curtidas
                </Badge>
            </div>
          </Card>
        )}
